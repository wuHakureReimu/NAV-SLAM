...
void nth_element(Point *points, size_t first, size_t last, size_t nth, int axis)
{
    ...
    for (size_t j = first; j < last; j++)
    {
        ...
        {
        case 0:
            cmp = points[j].x - pivot.x;
            break;
        case 1:
            cmp = points[j].y - pivot.y;
            break;
        case 2:
            cmp = points[j].z - pivot.z;
            break;
        }
        if (cmp <= 0)
        {
    ...
    if (i == nth)
        ...
        nth_element(points, i + 1, last, nth, axis);
    else
        nth_element(points, first, i - 1, nth, axis);
}
...
void nearestNeighborSearch(KDNode *root, Point *target, Point *result, double *bestDist, int depth)
{
    ...
    // 如果有可能，检查另一个子树
    if (fabs((axis == 0 ? target->x - root->point.x : axis == 1 ? target->y - root->point.y
